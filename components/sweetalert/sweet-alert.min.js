...
var defaultParams = {
  ...
  cancelButtonColor: '#B9B9B9',	//support cancel button color
  ...
  actionOnEnterKey: 'OK'		//若希望回车的时候默认执行cancle，请写CANCEL
};
...
var handleButton = function handleButton(event, params, modal) {
  ...
  //support cancel button color
  var normalColorCancel, hoverColorCancel, activeColorCancel;
  if (targetedCancel && params.cancelButtonColor) {
    normalColorCancel = params.cancelButtonColor;
    hoverColorCancel = _colorLuminance.colorLuminance(normalColorCancel, -0.04);
    activeColorCancel = _colorLuminance.colorLuminance(normalColorCancel, -0.14);
  }
  ...
  switch (e.type) {
    case 'mouseover':
      ...
      shouldSetCancelButtonColor(hoverColorCancel);
      break;

    case 'mouseout':
      shouldSetConfirmButtonColor(normalColor);
      shouldSetCancelButtonColor(normalColorCancel);
      break;

    case 'mousedown':
      shouldSetConfirmButtonColor(activeColor);
      shouldSetCancelButtonColor(activeColorCancel);
      break;
...
var handleKeyDown = function handleKeyDown(event, params, modal) {
  ...
  if (!(keyCode==9 || keyCode==13 || keyCode==32 || keyCode==27)){  //[9, 13, 32, 27].indexOf(keyCode) === -1) {
  ...
  if (keyCode === 9) {
    ...
    if (btnIndex === -1) {
      ...
      if ("CANCEL"==params.actionOnEnterKey){
...
var openModal = function openModal(callback) {
  ...
  var $okButton = $modal.querySelector('button.confirm');
  var $cancelButton = $modal.querySelector('button.cancel');
  if ($cancelButton && $modal.getAttribute("data-action-on-enter-key")=='CANCEL'){
  	$cancelButton.focus();
  }else{
  	$okButton.focus();
  }
...
var setParameters = function setParameters(params) {
  ...
  if (params.cancelButtonColor) {
    ...
    // Set the cancel button color to the loading ring
  ...
  modal.setAttribute('data-action-on-enter-key', params.actionOnEnterKey);
  /*
   * Animation
   */
  if (!params.animation) {
